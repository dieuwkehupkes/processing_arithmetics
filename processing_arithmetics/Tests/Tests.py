...
def test_DiagnosticClassifier_methods():
    _test_architecture_methods(DiagnosticClassifier, extra_classifiers=['subtracting'])
...
def _test_architecture_methods(architecture, extra_classifiers=None):
    ...
    A.add_pretrained_model(A.model, classifiers=extra_classifiers)
    ...
    training_data = A.generate_training_data({'L1':5, 'L3l':10, 'L4r':10}, pad_to=40, classifiers=extra_classifiers)

    # test generate training data from math treebank?
    m = MathTreebank({'L1':5, 'L3l':10, 'L4r':10}, digits=np.arange(-10, 11))
    validation_data = A.generate_training_data(m, pad_to=40, classifiers=extra_classifiers)
    ...
    test_data = A.generate_test_data(data=languages, digits=np.arange(-10, 11), pad_to=40, classifiers=extra_classifiers, test_separately=True)
