    ...
    def backprop(self, theta, delta, gradient, addOut=False, moveOn=True, fixWords=False, fixWeights=False):
        ...
        if addOut:  # add a delta message from its outputs (e.g., reconstructions)
            delta += np.concatenate(
                [out.backprop(theta, None, gradient, addOut, moveOn=False, fixWords=fixWords, fixWeights=fixWeights) for
                 out in self.outputs])
        M = theta[self.cat + ('M',)]
        if not fixWeights:
        ...
        if moveOn:
            ...
            [inputNode.backprop(theta, delt, gradient, addOut, moveOn, fixWords, fixWeights) for inputNode, delt in
             zip(self.inputs, np.split(deltaB, splitter))]
        else:
            return deltaB
...
class Leaf(Node):
    ...
    def forward(self, theta, activateIn=True, activateOut=False):
        ...
        self.a, self.ad = activation.activate(self.z, self.nonlin)
        if activateOut:
            [i.forward(theta, False, activateOut) for i in
             self.outputs]  # self.outputs.forward(theta, activateIn,activateOut)
