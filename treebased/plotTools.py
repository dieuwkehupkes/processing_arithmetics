...
def colorscale(n, color=None):
  if color is None: return plt.get_cmap('gist_rainbow', n)
  elif color =='red': return plt.get_cmap('autumn', n)
  elif color == 'blue': return plt.get_cmap('coolwarm', n)
  elif color == 'green': return plt.get_cmap('summer', n)
  elif color == 'purple': return plt.get_cmap('spring', n)
...
def scatterAndAnnotate(ax, point, color, label,size=12,txtcolor='black',pos=None):
  ...
  ha = 'left'
  va = 'center'
  if pos=='right': xy[0]+=0.06
  elif pos == 'left':
    xy[0]-=0.06
    ha = 'right'
  elif pos == 'top': xy[1]+=0.06
  elif pos =='bot':
    xy[0]-=0.2
    xy[1]-=0.15
  if label != '': an = ax.annotate(label, xy=xy,size=size, color=txtcolor,zorder = 10, ha=ha, va = va)
  else: an = None
  return [sc,an]
